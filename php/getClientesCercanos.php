...
if(!isset($_SESSION['ide']) || $_SESSION['ide'] == NULL){
    ...
    sin ( radians($user_lat) ) * sin( radians( lat ) ) ) ) AS distance
        ...
        STR_TO_DATE(ga.fecha_acepto,'%d/%m/%Y') <= DATE_SUB(NOW(), INTERVAL 2 MONTH)
        GROUP BY ga.primaryk
        HAVING distance <= ".$distancia." ORDER BY p DESC LIMIT 100";
    ...
    */    
    ...
    if(!$result){
        ...
        while ($fila = mysqli_fetch_array($result)) {
            ...
            STR_TO_DATE(llamadasProgramadas.fecha_llamada,'%d/%m/%Y') >= '$fecha_gestion' ORDER BY STR_TO_DATE(llamadasProgramadas.fecha_llamada,'%d/%m/%Y') ";
            $res=mysqli_query($enlace, $sql);
            //$arr_fechas = [];
            if( $res ){
