...
<body>
        ...
        <div class="container" style="max-width:96%!important; overflow-x:auto">
            <label><?php echo $operador; ?></label>
            <table class="table table-bordered table-dark" id="reporte">
                <thead class="thead-dark">
                    <tr>
                    <th scope="col" style="min-width:140px"><?php echo $str_tipo; ?></th>
...
if( $tipo == 'semanal'){
    ...
    $get_name = date('l', time()); //get week day
    ...
    $day_count = date('d');// Get the amount of days
    for ($i = 1; $i <= $day_count; $i++) {

        $date = $year.'/'.$month.'/'.$i; //format date
        $get_name = date('l', strtotime($date)); //get week day
        $day_name = substr($get_name, 0, 3); // Trim day name to 3 chars
        //echo '<br>'.$date.': '.$day_name.'<br>';
        //if not a weekend add day to array
...
$t_acepto = 0;
    ...
    $dias_trabajados = $dias_trabajados - $no_laborables;
