...
include "connect.php";
	...
	switch ($_REQUEST["accion"])
	{
		...
		case "consultar":
			...
			$stop_date = new DateTime($_REQUEST['start']);
			$d0 = $stop_date->format('d/m/Y');
			$stop_date->modify('+1 day');
			$d1 = $stop_date->format('d/m/Y');
			$stop_date->modify('+1 day');
			$d2 = $stop_date->format('d/m/Y');
			$stop_date->modify('+1 day');
			$d3 = $stop_date->format('d/m/Y');
			$stop_date->modify('+1 day');
			$d4 = $stop_date->format('d/m/Y');
			$stop_date->modify('+1 day');
			$d5 = $stop_date->format('d/m/Y');
			$stop_date->modify('+1 day');
			$d6 = $stop_date->format('d/m/Y');

			$hours = mysqli_query($enlace,"SELECT * FROM `Gestion_acepto` WHERE fecha_acepto IN ('".$d0."','".$d1."','".$d2."','".$d3."','".$d4."','".$d5."','".$d6."') ORDER BY hora_acepto ASC");
			$hours1 = mysqli_num_rows($hours);
			$i = 1;
			while($res = mysqli_fetch_array($hours))
			{
				if($i == 1)
				{
					$inicio_sem = $res['hora_acepto'];
				}
				else if($i == $hours1)
				{
					$fin_sem = $res['hora_acepto'];
				}
				else
				{
			...
			$inicio_sem = intval(substr($inicio_sem,0,2));
			$fin_sem = substr($fin_sem,0,2);
			$fin_sem = intval($fin_sem)+1;
            ...
            $colores = ["1"=>"green","2"=>"blue","3"=>"yellow","4"=>"#60cbd6","5"=>"red"];
            ...
            */
			...
			$fd2=str_pad($porciones[2], 2, "0", STR_PAD_LEFT);
                ...
                $newDate2 = strtotime ( '+'.$minutos.' minutes' , strtotime($fila["hora_acepto"]) ) ; 
				...
				$data[] = array(
					...
					'end'   => $fila["fecha"]."T".$fin,//10092021$fd.":59:".$fd2,
					'inicio'=>$inicio_sem,
					'fin'=>$fin_sem,
            ...
            //16082021 
            /*09092021 se elimina llamadas importantes del calendario $query = "SELECT ll.id,ll.realizada,ll.hora_llamada,STR_TO_DATE(ll.fecha_llamada,'%d/%m/%Y') AS fecha_llamada ,dc.codigo,dc.estado,dc.telefono,dc.nombre_completo
			 FROM `llamadasProgramadas` AS ll 
			 JOIN DatosClientes as dc on dc.codigo=ll.id_cliente 
