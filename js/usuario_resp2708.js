...
function cargar_gestion(cod_cliente = '') {
  ...
  $.post("../php/getAllClients.php", {
    ...
    for (var i = 0; i < cl.length - 1; i++) {
            var sc = cl[i].split("|");
            if( all_clientes.indexOf($.trim(sc[0])) == -1 ){//26082021
        
            cadena += ($.trim(sc[0]) + "|");
            all_clientes.push($.trim(sc[0]));
            }
    }
    obj_gestionados = [];
          ...
          if( !data.error ){
              if( data.data.length > 0 ){
                  for( let i = 0; i < data.data.length ; i++ ){
                          ...
                          let f = data.data[i];
                        if( all_clientes.indexOf($.trim(f.codigo)) == -1 ){//26082021
    
                          //2608 cadena += ($.trim(f.codigo) + "|");
                          cadena += ($.trim(f.codigo) + "|");
                          
                          obj_gestionados.push(f);
                          all_clientes.push(f.codigo);
                        }
                  }
              }
...
function llenar_clientes( desdeReporte ) {
  ...
  if(id_actual == '' ){ //20082021
    console.log('asignar id actual desde vector con ind:'+indice);
