...
var ult_codigo = '';//08092021 problema de gestionar todos los clientes con llamadas;
...
//27082021
   ...
   //09092021
   function limpiar_ref(){
       localStorage.removeItem('ultima_referencia');
       console.log('ref borradas');
   }
var date_limpieza_referencia = getItem('date_limpieza_referencia'); //09092021
console.log('date_limpieza_referencia:'+date_limpieza_referencia);
if( date_limpieza_referencia !== null ){
    console.log('comprobar hora');
    let obj_date_limpieza = new Date(date_limpieza_referencia);
    console.log('obj_date_limpieza');
    console.log(obj_date_limpieza);
    let ndate_actual = new Date();
    console.log('ndate_actual');
    console.log(ndate_actual);
    const diffTime = Math.abs(ndate_actual - obj_date_limpieza);
    const diffDays = parseInt(Math.abs(diffTime / (1000 * 60 * 60 * 24))); 
    console.log(diffTime + " milliseconds");
    console.log(diffDays + " days");
    if( diffDays > 0 ){
        console.log('limpiar referencias');
        localStorage.setItem('date_limpieza_referencia',ndate_actual);
    
        limpiar_ref();
    }
}
else{
    console.log('guardar date limpiar refs');
    let ndate = new Date();
    localStorage.setItem('date_limpieza_referencia',ndate);
    //limpiar_ref();
}
...
$("#guardarIdAct").click(function(){

  var oldc = document.getElementsByName('ocodigo');
  var newc = document.getElementsByName('ncodigo');


  var oldca = [].slice.call(oldc);
  var newca = [].slice.call(newc);

 var text = "";

  var ref = 1
  for (let i = 0; i < oldca.length; i++) {
    if(ref == oldca.length)
    {
      text += oldca[i].value;
    }
  ...
  for (let i = 0; i < newca.length; i++) {
    if(ref1 == newca.length)
    {
      text1 += newca[i].value;
    }
    else
  ...
  console.log(text);
  console.log(text1);
  $.post("../php/actualizar_nuevos.php", {
    ocodigo: text,
    ncodigo: text1
  }, function(mensaje) {
    $("#ModalNuevos").modal('toggle');
    if(mensaje == 1)
    {
      alert('Tiene códigos de extensión diferente a 16 caracteres.');
    }
    else if(mensaje == 2)
    {
      alert('Ingresó códigos duplicados, favor de verificar.');
    }
    else if(mensaje == 4)
    {
      alert('Ingrese almenos un código para actualizar.');
    }
    else if(mensaje == 5)
    {
      alert('Todos los registros fueron actualizados.');
    }
    else if(mensaje == 6)
    {
      alert('Fueron realizadas actualizaciones en los registros.');
    }

    $.post("../php/getActualizar.php", {}, function(mensaje) {

      var cl = mensaje.split("~");
      $("#botonNuevos").val('Clientes pendientes de código ('+cl[0]+')');
      document.getElementById('llenarNuevos').innerHTML = cl[1];      
    });
...
var dias_siguiente = 0;
var tiempo_peticion = 1000;//2000
var control_cargar_gestion = null;
function cargar_gestion(fecha_buscar='') {
  console.log('cargar gestion contador actual:'+contador);
  var respDes = desocuparCliente();
  respDes.done(function( mensaje) {
    
    //release_client(contador);
    ...
    $.post("../php/getAllClients_calendario_hoy.php", {
      ...
      let idx_cont=0;
      let obj_primer_cliente = null;
      for (var i = 0; i < cl.length - 1; i++) {
        var sc = cl[i].split("|");
        if( obj_primer_cliente === null ){
            obj_primer_cliente = {"idx":i,"codigo":$.trim(sc[0]),"coords":$.trim(sc[1]), "nombre":$.trim(sc[2]) ,"fecha":$.trim(sc[3]) ,
        "hora":$.trim(sc[4]),"int_hora": parseInt($.trim(sc[5])) };
        }
        json_clientes_hoy.push({"idx":i,"codigo":$.trim(sc[0]),"coords":$.trim(sc[1]), "nombre":$.trim(sc[2]) ,"fecha":$.trim(sc[3]) ,
        "hora":$.trim(sc[4]),"int_hora": parseInt($.trim(sc[5])), "hora_fin":$.trim(sc[6]) });
        //idx_cont++;
      }
      console.log('cli hoy');
      console.log(json_clientes_hoy);
      data_ultimo_cliente = getUltimoCliente();//09092021
      console.log('data_ultimo_cliente');
      console.log(data_ultimo_cliente);
      let ult_coords = (data_ultimo_cliente !== undefined && data_ultimo_cliente !== null)?data_ultimo_cliente.coords:'';
      ult_codigo = (data_ultimo_cliente !== undefined && data_ultimo_cliente !== null)?data_ultimo_cliente.codigo:'';
      let ult_nombre = (data_ultimo_cliente !== undefined && data_ultimo_cliente !== null)?data_ultimo_cliente.nombre:'';
      
      console.log('hoy ult coords:'+ult_coords+'-ult_codigo:'+ult_codigo+'--'+ult_nombre);
      console.log('ultimo cli');
      console.log(data_ultimo_cliente);
      //console.log('hoy:'+(idx_cont-1));
      if(data_ultimo_cliente.buscar_sig){
          console.log('buscar sig dia');
          console.log('dias_siguiente:'+dias_siguiente);
          if(dias_siguiente > 7) {
            alert("No se encontraron clientes en los próximos 7 días");
            $(".loader").fadeOut("slow");
            return;
          }
          dias_siguiente++;
          console.log('dias_siguiente:'+dias_siguiente);
          setTimeout(function(){ cargar_gestion() }, tiempo_peticion);
          console.log('despues de '+tiempo_peticion+' se traen clientes');
          return;
      }
          ...
          data = JSON.parse(data);
            if( !data.error ){
                if( data.data.length > 0 ){
                    let limite_gestionados = (data.data.length <= 3)?data.data.length:3;//03092021 211020212021
                  ...
                  $.post("../php/getAllClients_calendario.php", {
                      ...
                      coords: ult_coords
                        ...
                        if( !data.error ){
                              ...
                              let num_por_gestionar = 0;//09092021
                              for (var i = 0; i < data.data.length; i++) {
                                  var sc = data.data[i];
                                
                                  cadena += ($.trim(sc.codigo) + "|");
                                  let obj_find = all_clientes.find(o => o.codigo == $.trim(sc.codigo) );//03092021
              
                                  if( obj_find === undefined ){//26082021
                                      //07092021 comprobar que no esté en clientes gestionados
                                      let idx_gestionado = arr_codigos_tabla.indexOf( $.trim(sc.codigo) );
                                     // console.log('idx_gestionado:'+idx_gestionado);
                                      if( idx_gestionado == -1 ){
                                        //  console.log('no está gestionado agregar');
                                          cadena_por_gestionar += ($.trim(sc.codigo) + "|");
                                          num_por_gestionar ++;//09092021
                                          all_clientes.push({'codigo':$.trim(sc.codigo),'nombre_completo':$.trim(sc.nombre_completo),'posicion':posicion_futura,'desatendido':$.trim(sc.desatendido)} );//03092021
                                          posicion_futura++;
                                      }
                                  }
                              }
                              //09092021 si no hay clientes, pasar referencia a siguente
                              if( num_por_gestionar == 0 ){
                                  alert('No hay clientes por gestionar para:'+ult_nombre);
                                  console.log('no hay clientes guardar ult refe:'+ult_codigo+'-n:'+ult_nombre);
                                  let arr_referencias = getItem('ultima_referencia');
                                  console.log('arr_referencias:'+arr_referencias);
                                  if( arr_referencias !== null){
                                      arr_referencias = arr_referencias.split(",");
                                  }
                                  else{
                                      arr_referencias = [];
                                  }
                                  console.log('add ult_codigo:'+ult_codigo);
                                  arr_referencias.push(ult_codigo);
                                    console.log('--------');
                                    arr_referencias = arr_referencias.toString();
                                    console.log(arr_referencias);
                                    
                                    localStorage.setItem('ultima_referencia',arr_referencias);
                                      console.log('volver a cargar gestion');
                                  cargar_gestion();
                                  return;
                              }
                              //fin  09092021
...
function llenar_clientes_2( desdeReporte ) {
  ...
  let desatend = obj_find_actual.desatendido;
  console.log('Desatendido='+desatend)
  ...
  $.post('is_managing.php', {
    ...
    $.when(
      ...
      function procesar(data){
        ...
        html_nuevo_view+='<td class="" data-codigo=""></td>';
        //09112021 html_nuevo_view+='<td class="col_blanco" data-codigo=""></td>';
...
function llenar_clientes( desdeReporte ) {
    desocuparCliente();//27082021
    console.log('llenar_clientes contador:'+contador+'-id actual:'+id_actual);
    arr_codigos_tabla = [];
  // Si no se ha gestionado en los botones se guarda como tiempo muerto, de lo contrario no.
  $.when(function() {
    setTimeout(function() {}, 1000);
  ...
  console.log("contador 545:"+contador);
  var indice = contador;
  ...
  if(id_actual == '' ){ //20082021
    console.log('asignar id actual desde vector con ind:'+indice);//27082021
      ...
      if( contador > -1 ){
          console.log('buscar segun contador');
          id_actual = obtenerIdActual(indice);//27082021;
      }
  }
...
function llenar_llamadas() {
  ...
  $.post("../php/getClientsByUser_recordatorio.php", {
    ...
    for (var i = 0; i < cl.length - 1; i++) {
      ...
      console.log('color:'+sc[5]);
      //console.log('come:'+sc[6]);
      /*
      var clase_color = '';
      if(sc[5] == 'Volver a llamar Importante'){
          clase_color = "style='background-color: #85DA91";
      }
      console.log('clase_color:'+clase_color);
      */
     if(sc[5] == 'Volver a llamar Importante')
     {
  ...
  $("#registrosD tbody").delegate('tr', 'click', function() {
    ...
    console.log("identificador es:"+identificadorU);
...
function gestionar_urgente(id_cliente) {
  ...
  $.when($.post("../php/obtenerDatosCliente.php", {
    ...
    console.log("este es mensaje: "+w[23]);
...
$("#GuardarA").click(function() {
  ...
  if ($("#fechaAcepto").val() == "") {
  ...
  else if ($("#minutoAC").val() == "") {
    alert("minuto es obligatorio!");
    $("#GuardarA").removeClass('disabled');
    $("#GuardarA").html("Guardar");
  }
  else {
    ...
    let response_data = guardar_tiempo('ac', cliente);//16092021 //fred ssss
                      ...
                      comentario: comentario
                        ...
                        llenar_llamadas();
                            if (urge) {
                              ...
                              urge = false;
                              //11082021
                              console.log('elimina gestionModal urgente:gestionModal_'+cliente);
                              cerrarModal(cliente);
                              
                              //$("#gestionModal").modal('toggle');
                            }
                        $("#mensaje_error_acepto").addClass('hide');
                        sumar_gestion('ac', cliente);
...
if ($("#fechaAceptoCom").val() == "") {
  ...
  if(response_data.data == 1){
      ...
      $.post("../php/valida_hora_aceptoCom.php", {
  ...
  if ($("#fechaRechazo").val() == "") {
    ...
    $.post("../php/Cliente_rechazo.php", {
      ...
      if (mensaje == 1) {
        ...
        cargar_gestion();//llenar_clientes_2()//21102021
        /*21102021 if (!urge) {
          llenar_clientes_2();
        }*/
        ...
        gestionado_botones = true;
        guardar_tiempo('re', cliente);//16092021
...
function bloquear_urgente() {
  ...
  setTimeout(function() {
    ...
    var h = f.getHours();
    var m = f.getMinutes();
    console.log('------arr_id_gestionar');//21102021
    console.log(arr_id_gestionar);//21102021
...
$("#GestionarC").click(function(event) {
  ...
  if (cl==="0x000" || cl=="") {
   ...
   // $("#GestionarC").removeClass('disabled');
    ...
    cambiarActual(cl,temp_es_anterior);//21102021
    release_client_id(cl,true);
    //12102021 cambio_siguiente = true;
       ...
       setTimeout(() => {
          actualizarCookie(1);
            ...
            if( id !== undefined ){
                ...
                console.log('nueva pos:'+up)
                all_clientes.push({'codigo':cl, 'nombre_completo':nombre_buscado,'posicion':up});
                cambio_siguiente = true;//12102021
            }
            cambiarActual(cl);
            release_client_id(cl,true);
            $(".search").fadeOut("fast");
            
            //12102021cambio_siguiente = true;
            primer_enter = false;
...
$("#input_BCC").change(function() {
  ...
  usuario = $("#input_BCC").val();
    ...
    nombre_buscado = $("#input_BCC option:selected").html();//10092021 20092021
  console.log(nombre_buscado);//10092021
       ...
       hizo_busqueda = true;//12102021
        ...
        $(".search").fadeIn("fast");
            if( id !== undefined ){
                ...
                console.log('cambio_siguiente enter:'+cambio_siguiente)//12102021
                console.log('llamar cambiaractual');
            }
            else{
              console.log('se agrega a arr ');
              ...
              //24/06/22 MG esto evita que la lupa se quede cargando siempre
...
//modal minimizado
    ...
    $("body").on("click",'.col_siguiente',function(e){
        ...
        console.log('id_actual:'+id_actual);
        //21102021
        console.log('esta_cambiando:'+esta_cambiando);
        if(esta_cambiando){
            console.log('aún está cambiando');
            return;
        }
        esta_cambiando = true;
        console.log('esta_cambiando:'+esta_cambiando);
    ...
    function getIndiceSiguiente(idx){
        ...
        for(let i = idx ; i < vector.length ; i++ ){
            console.log('i:'+i);
            if( vector[i] != "" && arr_codigos_tabla.indexOf(vector[i]) == -1 || estaGestionado(cod)){
    ...
    $(document).on('click','.fila_filtro',function(){
        ...
        //22092021$('#modal_filtro').modal('hide');
        //22092021$('#tb_resultados_filtro tbody').html('');
    });
...
//inspecciones
     $("#GuardarInspeccion").click(function() {
        ...
        $("#GuardarInspeccion").addClass('disabled');
          ...
          if ($("#fechaAceptoInspeccion").val() == "") {
          ...
          else if ($("#horaACInspeccion").val() == "") {
            alert("La hora es obligatoria!");
            $("#GuardarInspeccion").removeClass('disabled');
            $("#GuardarInspeccion").html("Guardar");
          }
          else if ($("#minutoACInspeccion").val() == "") {
            alert("minuto es obligatorio!");
            $("#GuardarInspeccion").removeClass('disabled');
            $("#GuardarInspeccion").html("Guardar");
          }else {
            ...
            comentario = $("#commentInspeccion").val();
                ...
                let response_data = guardar_tiempo('in', cliente);//14082021 fred ss
...
function guardar_otras_gestiones(id_modal, id_txt_gestion ,hora, fecha,comentario, tipo_gestion){
    ...
    $("#fechaAceptoCobros").change(function() {
     ...
     $("#GuardarCobros").click(function() {
        ...
        $("#GuardarCobros").addClass('disabled');
          ...
          if ($("#fechaAceptoCobros").val() == "") {
          ...
          else if ($("#horaACCobros").val() == "") {
            alert("La hora es obligatoria!");
            $("#GuardarCobros").removeClass('disabled');
            $("#GuardarCobros").html("Guardar");
          }
          else if ($("#minutoACCobros").val() == "") {
            alert("minuto es obligatorio!");
            $("#GuardarCobros").removeClass('disabled');
            $("#GuardarCobros").html("Guardar");
          }
            ...
            let response_data = guardar_tiempo('co', $("#cliente_selc_pargestion").val());//1808 fred
    ...
    $('#sel1').on('select2:open', function (e) {
        $('.select2-dropdown--below').addClass('normalLeft normal_width');
    ...
    function getUltimoCliente(){
        ...
        let ultimo_cliente = null;
        //09092021
        let ult_ref = '';
        ultimo_codigo = getItem('ultima_referencia');
        console.log('ultimo_codigo:'+ultimo_codigo);
        if(ultimo_codigo !== null ){
            ultimo_codigo = ultimo_codigo.split(",");
        }
        else{
            ultimo_codigo = [];
        }
        ...
        //fin 09092021
        let obj_anterior_fecha = null;
        let last_client = null;
        let hora_sistema = new Date();
        let int_hora_sistema = hora_sistema.getHours();
        let min_hora_sistema = hora_sistema.getMinutes();
        console.log('sit:'+hora_sistema+'----hora:'+int_hora_sistema+'--'+min_hora_sistema);
        for ( let i = 0 ; i < json_clientes_hoy.length ; i ++){
            let cli_hoy = json_clientes_hoy[i];
            
            let arr_fecha = cli_hoy.fecha.split("/");
            arr_fecha = arr_fecha[1]+'/'+arr_fecha[0]+'/'+arr_fecha[2];
            console.log('new f:'+arr_fecha+' '+cli_hoy.hora+'-'+cli_hoy.hora_fin);
            let obj_date = new Date(arr_fecha+' '+cli_hoy.hora);
            let obj_date_fin = new Date(arr_fecha+' '+cli_hoy.hora_fin);
            console.log(obj_date);
            let date = new Date(arr_fecha);
            date.setHours(int_hora_sistema);
            date.setMinutes(min_hora_sistema);
            console.log('sistema:'+date);
            console.log('h sistema:'+date.getHours())
            if( obj_anterior_fecha !== null ){
                console.log(obj_anterior_fecha);
                let diffInMilliSeconds = Math.ceil(obj_date - obj_anterior_fecha) / 1000;
                console.log('diffInMilliSeconds:'+diffInMilliSeconds);
                let dif_horas = Math.floor(diffInMilliSeconds / 3600) % 24;
                console.log('dif_horas:'+dif_horas);
                
                let diff_actual_miliseconds = Math.ceil(obj_date - date) / 1000;
                console.log('diff diff_hora_actual:'+diff_actual_miliseconds);
                let dif_horas_actual = Math.floor(diff_actual_miliseconds / 3600) % 24;
                console.log('dif_horas_actual:'+dif_horas_actual);
                console.log('ultimo_codigo:'+ultimo_codigo);
                console.log('--buscar '+cli_hoy.codigo);
                //09092021 if( dif_horas >= 1 && dif_horas_actual >=1 && cli_hoy.codigo != ultimo_codigo ){
                let idx_ult_cod = ultimo_codigo.indexOf(cli_hoy.codigo);
                console.log('idx_ult_cod:'+idx_ult_cod);
                if( dif_horas >= 1 && dif_horas_actual >=1 && idx_ult_cod == -1 ){
                    console.log('referencia encontrada comprobar si anterior ya no tenía clientes:'+last_client.codigo+'-'+last_client.nombre);
                    let idx_ant_sc = ultimo_codigo.indexOf(last_client.codigo);;
                    console.log('idx_ant_sc:'+idx_ant_sc);
                    if( idx_ant_sc == -1 ){
                        console.log(last_client);
                        ultimo_cliente = last_client;
                        ultimo_cliente.buscar_sig = false;//20092021
                        ult_ref = cli_hoy.codigo; //09092021
                        break;
                    }
                    else{
                        console.log('este ref:'+last_client.codigo+'-'+last_client.nmbre+' n tenía clientes pasa al siguiente')
                    }
                }
                else{
                    last_client = cli_hoy;
                    obj_anterior_fecha = obj_date_fin;
                }
                
            }
            else{
                console.log('compara con hora actual primer cliente');
                console.log(obj_date);
                console.log('ahora');
                console.log(date);
               
                const hora_laborable = new Date(arr_fecha);
                hora_laborable.setHours(8);
                console.log('hora laborable ');
                console.log(hora_laborable);
                let diff_actual_miliseconds = Math.ceil(obj_date - date) / 1000;
                console.log('diff diff_hora_actual 2:'+diff_actual_miliseconds);
                let dif_horas_actual = Math.floor(diff_actual_miliseconds / 3600) % 24;
                console.log('dif_horas_actual  2:'+dif_horas_actual);
                console.log('ultimo_codigo:'+ultimo_codigo);
                
                let diff_actual_miliseconds_l = Math.ceil(obj_date - hora_laborable) / 1000;
                console.log('diff diff_hora_actual lab 2:'+diff_actual_miliseconds_l);
                let dif_horas_actual_l = Math.floor(diff_actual_miliseconds / 3600) % 24;
                console.log('dif_horas_actual lab 2:'+dif_horas_actual_l);
                
                console.log('--buscar '+cli_hoy.codigo);
                let idx_ult_cod = ultimo_codigo.indexOf(cli_hoy.codigo);
                console.log('idx_ult_cod:'+idx_ult_cod);
                if(diff_actual_miliseconds_l > 0 && dif_horas_actual_l >=1 && dif_horas_actual >=1 && idx_ult_cod == -1 ){
                    console.log('referencia 2 encontrada');
                        ultimo_cliente = cli_hoy;
                        ultimo_cliente.buscar_sig = false;//20092021
                        ult_ref = cli_hoy.codigo; //09092021
                        break;
                    ...
                    console.log('continua con siguiente');
                    last_client = cli_hoy;
                    obj_anterior_fecha = obj_date_fin;
        ...
        if( ultimo_cliente === null && json_clientes_hoy.length > 0 ){
             
            let ucli = json_clientes_hoy[json_clientes_hoy.length - 1];
            console.log(ucli);
            let arrhfin = ucli.hora_fin.split(":");
            console.log(arrhfin);
            let int_fin_hora = parseInt(arrhfin[0]);
            console.log('int_fin_hora:'+int_fin_hora);
            int_fin_hora++;
            console.log('int_fin_hora:'+int_fin_hora);
            
            if(int_fin_hora < 17 ){
                console.log('asignar ultimo cliente');
                 ultimo_cliente = json_clientes_hoy[json_clientes_hoy.length - 1];
                ult_ref = ultimo_cliente.codigo; //09092021
            }
            else{
                console.log('buscar siguiente día ')
                ultimo_cliente = {buscar_sig:true}
            }
        }
        else if(ultimo_cliente === null){
        ...
        /*for ( let i = hora ; i <= 23 ; i++){//18 26082021
             for( let j= 0; j < json_clientes_hoy.length ; j++ ){
                 let aux_obj = json_clientes_hoy[j];
                 console.log('cli:'+j);
                 console.log(aux_obj);
                 if( aux_obj.int_hora == i ){
                     ultimo_cliente = aux_obj;
                     break;
                 }
                 else if(aux_obj.int_hora > i ){
    ...
    function resetPosicionesSiguiente(){
        ...
        for( let i = 4 ; i < 12 ; i++ ){
            ...
            if( index_f > -1 ){
                ...
                if( all_clientes[index_f].posicion_anterior !== undefined && all_clientes[index_f].posicion_anterior != all_clientes[index_f].posicion ){//06092021
    ...
    function resetPosicionesAnterior(){
        ...
        for( let i = 1 ; i <= 12 ; i++ ){
            ...
            if( index_f > -1 ){
                console.log(' reset for:'+i+' de '+all_clientes[index_f].posicion+' a '+all_clientes[index_f].posicion_anterior);
                if( all_clientes[index_f].posicion_anterior !== undefined && all_clientes[index_f].posicion_anterior != all_clientes[index_f].posicion){//06092021 21102021
